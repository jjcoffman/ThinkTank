...
public class Cichlid extends Fish implements IMoving, PhysicsCollisionGroupListener {
	...
	/*
	 * These variables provide weights based on these attributes and what impact
	 * they have on interacting with other fish
	 */
	...
	/**
	 * Constructor for creating a cichlid object with the specified values for
	 * size, speed, and sex.
	 * 
	 * @param size
	 *            the size value for this cichlid.
	 * @param speed
	 *            the speed value for this cichlid.
	 * @param sex
	 *            the set value for this cichlid.
	 */
	 ...
	 *            the speed value for this cichlid.
	 * @param sex
	 *            the sex value for this cichlid.
	 * @param name
	 *            the name for this cichlid
	 */
	...
	// GETTERS
	/**
	 * Returns the <code>IStrategy</code> used by the cichlid to determine its
	 * behavior.
	 * 
	...
	// SETTERS
	/**
	 * Sets the <code>IStrategy</code> to be used by this cichlid to the
	 * specified object.
	 * 
	 * @param strat
	 *            the object to use for this cichlid's behavior.
	 */
	...
	// OPERATIONS
	/**
	 * Initializes the values of this <code>Cichilid</code> object and prepares
	 * it to be displayed in the environment.
	 */
	private void init() {
		fish = new Node();
		...
		Material cichlidMat = new Material(Main.am, "Common/MatDefs/Misc/Unshaded.j3md");
		cichlidMat.setTexture("ColorMap", Main.am.loadTexture(new TextureKey("Cichlid/CichlidText.jpg", false)));
	 ...
	 * 
	 * @param control
	 *            the <code>AbstractControl</code> object to be added.
	 */
	...
	/**
	 * Calculates and sets the values for the dimensions of this cichlid in the
	 * environment.
	 */
	...
	/**
	 * This Sets the Collision Group parameters for the Cichlid Fish object and
	 * assigns it the proper collision parameters to collide with other Fish,
	 * the Tank, and Plants.
	 * 
	 * @param GhostControl
	...
	private void setCollisionGroups(GhostControl ghost2) {
		ghost2.setCollisionGroup(PhysicsCollisionObject.COLLISION_GROUP_01); // collision
																				// group
																				// 1
																				// is
																				// cichlid
		ghost2.setCollideWithGroups(PhysicsCollisionObject.COLLISION_GROUP_01);
		ghost2.addCollideWithGroup(PhysicsCollisionObject.COLLISION_GROUP_02); // collision
																				// group
																				// 2
																				// is
																				// Tank
		ghost2.addCollideWithGroup(PhysicsCollisionObject.COLLISION_GROUP_03); // collsion
																				// group
																				// 3
																				// are
																				// plants
	...
	/**
	 * This is the handler for behavioral movement. It receives the scenario
	 * object and handles iteration through all the objects in the tank and
	 * calls independent interaction methods for each item.
	 * 
	 * @param Scenario
	 *            scenario
	 */
	private void behavioralMovement(float tpf) {
		...
		// Here determine which fish is a target fish. If none targetAggression
		// will remain 0
		while (itrF.hasNext()) {
		...
		// This has to be here so that it fish interaction occurs first and
		// takes into account
		while (itrO.hasNext()) {
		...
		/*
		 * Here we handle the interactions between the Fish. We comapare the two
		 * that are the aggressors and invoke attack or run methods to hide
		 */
	...
	private void hide(EnvironmentObject shelterObject, float tpf) {
		/*
		 * TODO similar to run but only NEAR the object and it has to remain on
		 * a side So you cant do a rand num and determine + or - to decide what
		 * side to go to Have to make it relative to fish
		 */
		...
		// here we increase the speed a little bit to encourage a more realistic
		// scenario.
	...
	/**
	 * This Cichlid will Run from the attacking fish in the tank. Pulled the
	 * avoiding point details from the target fish who is more aggressive that
	 * the running fish
	 * 
	 * @param tpf
	 */
	...
	/**
	 * DO NOT CALL DIRECTLY: Use behavioralMovement() Handles the interaction
	 * with the cichlid object and the fish.
	 * 
	 * @param EmvironmentObject
	 *            next
	 * @return Double shelterWeight
	 */
	private double objectInteract(EnvironmentObject next) {
		double shelterWeight = 0;
		// TODO Needs to attract the fish without causing harm and the fish will
		// remain close
		return shelterWeight;
	}

	/**
	 * DO NOT CALL DIRECTLY: Use behavioralMovement() Handles the interations
	 * with other fish via range with a weight, size with a weight, and speed
	 * with a weight
	 * 
	 * @param Fish
	 *            opponent
	...
	private double fishInteract(Fish opponent) {
		...
		if (!this.getSex().matches(opponent.getSex()))
			aggression = aggression * 2; // This will accoint for different
											// sex's with an attemot to mate
		aggression = aggression * calculateRelationships(opponent).getVisibility(); // here
																					// we
																					// account
																					// for
																					// visibility
	...
	/**
	 * Used by avoidance algorithm to determine relative positions of colliding
	 * fishes
	 * 
	 * @param pos
	 *            this objects own coordinate position
	 * @param avoid
	 *            colliding fish's coordinate position
	 * @param g
	 *            this objects position on the grid
	...
	/**
	 * Used by Desited algorithm to determine relative positions of colliding
	 * fishes
	 * 
	 * @param pos
	 *            this objects own coordinate position
	 * @param Desired
	 *            Targets coordinate position
	 * @param g
	 *            this objects position on the grid
	 * @return new position to move to, on the grid
	 */
	...
	private Vector3f getNextLoc(float tpf) {
		...
		movement = new Vector3f(0, 0, tpf * getSpeed()); // TODO add multiplier
															// for fast forward
	...
	}// end of readObjectNoData method
